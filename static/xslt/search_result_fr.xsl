...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="sort_hits">sort_hits_f.gif</xsl:variable>
	<xsl:variable name="sort_year">sort_year_f.gif</xsl:variable>
	<xsl:variable name="sort_AZ">sort_AZ_f.gif</xsl:variable>
	<xsl:variable name="HS_hide">HS_hide_f.gif</xsl:variable>
	<xsl:variable name="HS_see">HS_see_f.gif</xsl:variable>
	...
	<xsl:template match="/">
		...
		<xsl:choose>
			<xsl:when test="$resultCount &gt; 0">
				...
				<div class="left">
					<form class="easyform" name="Search" action="results" method="get">
						...
						<fieldset>
							<legend>Recherche dans ces résultats</legend>
							...
							<ul class="options">
								...
								<li class="option-type-genealogy">
									<xsl:call-template name="mediaType">
										...
										<xsl:with-param name="mtLabel">Ressource genealogique</xsl:with-param>
									</xsl:call-template>
								</li>
			...
			<xsl:otherwise>
				<div class="noResults">Aucun résultat n'a été trouvé.<br/>
					<!-- try to relax the constraints (OR) -->
					<xsl:element name="a">
						<xsl:attribute name="href">
							<!--xsl:text>resultat?</xsl:text><xsl:value-of select="$unparsedQuery"/>&amp;bl=or-->
							<xsl:text>resultat?</xsl:text>
						</xsl:attribute> Essayez cette recherche à nouveau avec des critères élargis! (<a href="aide#relax">Aide</a>)</xsl:element>
					<br/>
					<!-- try a fuzzier query -->
					<xsl:element name="a">
						<xsl:attribute name="href">
							<!--xsl:text>resultat?</xsl:text><xsl:value-of select="$unparsedQuery"/><xsl:text>&amp;fz=1</xsl:text-->
							<xsl:text>resultat?</xsl:text>
						</xsl:attribute> Essayez cette recherche à nouveau: plus floue!(<a href="aide#floue">Aide</a>)</xsl:element>
					<br/>
					<form action="results" method="get" class="SearchForm" name="Search"> Nouvelle recherche: <input type="text" name="q" size="50"/>
	...
	<div class="searchLogic">
		<xsl:if test="//altTermFromCollation">
			<div class="alert">
				Votre recherche de <b><xsl:value-of select="//query" /></b> n'a donné aucun résultat. Nous l’avons remplacée par <b><xsl:value-of select="//altTermFromCollation" /></b>.<br/>
			</div>
		</xsl:if>
		<xsl:choose>
			<xsl:when test="$resultCount = 1">
				Votre requête a produit <b class="count">1</b> résultat.<br />
			</xsl:when>
			<xsl:otherwise>
				Votre requête a produit <b class="count"> <xsl:value-of select="//result/@numFound"/></b> résultats.<br />
			</xsl:otherwise>
		</xsl:choose>
	</div>
	</xsl:template>
	...
	<xsl:template name="docParts">
		<xsl:if test="./parts/response/result/@numFound &gt; 0">
			...
			<div class="docParts">
				<div class="docPartsLabel">Pages/pièces:</div>
	...
	<xsl:template name="paging">
		...
		<span>Page <xsl:value-of select="$p"/> de <xsl:value-of select="$tp"/>
		</span>
		...
		<xsl:choose>
			<xsl:when test="($p - 1) &gt; 0">
				<xsl:element name="a">
					<xsl:attribute name="href"><xsl:text>resultat?</xsl:text><xsl:value-of
							select="$unparsedQuery"/>&amp;p=<xsl:value-of select="$p - 1"/>
					</xsl:attribute> &#x2190; Ensuite </xsl:element>
			</xsl:when>
		</xsl:choose>
		...
		<xsl:choose>
			<xsl:when test="($p + 1) &lt; $tp + 1">
				<xsl:element name="a">
					<xsl:attribute name="href"><xsl:text>resultat?</xsl:text><xsl:value-of
							select="$unparsedQuery"/>&amp;p=<xsl:value-of select="$p + 1"/>
					</xsl:attribute> Suivant &#x2192;</xsl:element>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="mediaType">
		...
		<xsl:choose>
			<xsl:when test="//lst[@name='type']/int[@name=$mt]">
				<xsl:element name="a">
					<xsl:attribute name="href"><xsl:text>resultat?mt=</xsl:text><xsl:value-of
	...
	<xsl:template name="Location">
		<xsl:for-each select="//lst[@name='fSpatial']">
			<legend>Lieux géographiques</legend>
			<xsl:choose>
	...
	<xsl:template name="LocationSimple">
		<xsl:for-each select="./int">
			<xsl:if test="normalize-space(./@name)">
				<xsl:element name="li">
					<xsl:element name="a">
						<xsl:attribute name="href"><xsl:text>resultat?lc=</xsl:text><xsl:value-of
	...
	<xsl:template name="LocationBrief">
		...
		<xsl:element name="li">
			<xsl:element name="a">
				<xsl:attribute name="href">
					<xsl:text>resultat?lcmore=1&amp;</xsl:text>
				...
				<xsl:element name="img">
					...
					<xsl:attribute name="alt">Afficher tout</xsl:attribute>
				</xsl:element>
			</xsl:element>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template name="LocationComplex">
		<xsl:element name="a">
			<xsl:attribute name="href">
				<xsl:text>resultat?lcmore=0&amp;</xsl:text>
				<xsl:call-template name="replace-string">
					<xsl:with-param name="text" select="$unparsedQuery"/>
			...
			<xsl:element name="img">
				...
				<xsl:attribute name="alt">Cacher</xsl:attribute>
			</xsl:element>
		</xsl:element>
	...
	<xsl:template name="UnidentifiedLocationCount">
		...
		<xsl:if test="$resultFinal &gt; 0">
			<div class="facetpanelcontentelement">Lieu géographique est indisponible: <xsl:value-of
					select="$resultFinal"/>
			</div>
		</xsl:if>
	</xsl:template>
				...
				<div class="tagcloud rightBox">
					...
					<xsl:for-each select="./int">
						...
						<xsl:if test="normalize-space(./@name)">
							<xsl:element name="a">
								<xsl:attribute name="href"
										><xsl:text>resultat?itype=</xsl:text><xsl:value-of
					...
					<xsl:for-each select="./int">
						<xsl:if test="./@name = 'true'">
							<xsl:element name="li">
								<xsl:element name="a">
									<xsl:attribute name="href"
											><xsl:text>resultat?fc=true</xsl:text>&amp;<xsl:value-of
											select="$unparsedQuery"/>
									</xsl:attribute>
									<xsl:element name="img"><xsl:attribute name="src"><xsl:value-of
												select="$icon_comment"
											/></xsl:attribute><xsl:attribute name="alt"
											><xsl:text>commentaires</xsl:text></xsl:attribute></xsl:element>&#160;<xsl:text>Commentaires</xsl:text>&#160;(<xsl:value-of
										select="."/>) </xsl:element>
							</xsl:element>
						</xsl:if>
					</xsl:for-each>
					...
					<xsl:for-each select="./int">
						<xsl:if test="./@name = 'true'">
							<xsl:element name="li">
								<xsl:element name="a">
									<xsl:attribute name="href"
											><xsl:text>resultat?fm=true</xsl:text>&amp;<xsl:value-of
											select="$unparsedQuery"/>
									</xsl:attribute><xsl:element name="img"><xsl:attribute
											name="src"><xsl:value-of select="$icon_mystery"
											/></xsl:attribute><xsl:attribute name="alt"
											><xsl:text>mystères</xsl:text></xsl:attribute></xsl:element>&#160;<xsl:text>Mystères</xsl:text>&#160;(<xsl:value-of
										select="."/>) </xsl:element>
							</xsl:element>
						</xsl:if>
					</xsl:for-each>
	...
	<xsl:template name="Source">
		<xsl:for-each select="//lst[@name='fSource']">
			<legend>Ministère/Agence</legend>
			<xsl:choose>
	...
	<xsl:template name="GroupName">
		<xsl:for-each select="//lst[@name='fGroupName']">
			<legend>Groupes/Collections</legend>
			<xsl:choose>
	...
	<xsl:template name="translate">
		<center>
			<xsl:element name="script">
				<xsl:attribute name="src"
					>http://www.gmodules.com/ig/ifr?url=http://www.google.com/ig/modules/translatemypage.xml&amp;up_source_language=fr&amp;w=160&amp;h=60&amp;title=&amp;border=&amp;output=js</xsl:attribute>
			</xsl:element>
		</center>
	</xsl:template>
							...
							<xsl:element name="li">
								<xsl:element name="a">
									...
									<xsl:element name="img">
										...
										<xsl:attribute name="alt">Tri par occurrences</xsl:attribute>
									</xsl:element>
								</xsl:element>
							</xsl:element>
						</xsl:when>
									...
									<xsl:choose>
										<xsl:when test="contains($listName, 'year')">
											<xsl:element name="img">
												...
												<xsl:attribute name="alt">Tri par année</xsl:attribute>
											</xsl:element>
										</xsl:when>
										<xsl:otherwise>
											<xsl:element name="img">
												...
												<xsl:attribute name="alt">Tri par A-Z</xsl:attribute>
											</xsl:element>
										</xsl:otherwise>
									</xsl:choose>
								</xsl:element>
							...
							<xsl:for-each select="./int">
								...
								<xsl:if test="normalize-space(./@name)">
									<xsl:element name="li">
										<xsl:element name="a">
											<xsl:attribute name="href"
												><xsl:text>resultat?</xsl:text><xsl:value-of
												select="$listName"
